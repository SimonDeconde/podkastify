...
import {
  ...
  RequestMagicLinkResponse as RequestMagicLoginLinkResponse,
  UserLoginResponse,
} from '@server/user/user.types';
import { Roles, UserById } from '@shared/interfaces';
import generator from 'generate-password-ts';
import _ from 'lodash';
import {
  QuickLoginDtoType,
  RequestMagicLoginLinkDtoType,
  UserCreateDtoType,
  UserFindAllDtoType,
  UserLoginDtoType,
...
export class UserService {
  ...
  async requestMagicLoginLink(
    requestMagicLoginLinkDto: RequestMagicLoginLinkDtoType,
  ): Promise<RequestMagicLoginLinkResponse> {
    const user = await this.prismaService.user.findFirst({
      where: {
        email: requestMagicLoginLinkDto.email,
        deletedAt: null,
      },
    });
    ...
    if (user) {
      ...
      await this.emailService.sendMagicLoginLink(user);
    }

    // Returning success regardless of whether the email exists or not, for security reasons.
    return {
      status: RequestMagicLinkStatus.Success,
    };
  }

  async quickLogin(
    quickLoginDto: QuickLoginDtoType,
  ...
  async create(userCreateDto: UserCreateDtoType) {
    ...
    if (!userCreateDto.profilePicUrl) {
      ...
      // );
      userCreateDto.profilePicUrl =
        'https://i.pravatar.cc/150?u=' + userCreateDto.email;
    }

    // create
    try {
      const { isVerified, ...userData } = userCreateDto;
  ...
  async update(userUpdateDto: UserUpdateDtoType) {
    ...
    let roleConnections: { id: string }[] = [];
    let roleDisconnections: { id: string }[] = [];
    if (
  ...
  async resetPassword(resetPasswordDto: UserResetPasswordType) {
    ...
    if (!user) {
      // No-op if user not found, we don't want to leak user existence.
      return null;
    }
